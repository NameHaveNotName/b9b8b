...
function imageProviderPreferenceHeaders(p: GenerateImageParams): Record<string, string> {
  const preferStable = p.providerPreference === 'stable' || p.model.startsWith('gpt-image-')
...
class OpenLuxHttpError extends Error {
  status: number
  body: string
  constructor(status: number, body: string) {
    super(`OpenLux API ${status}: ${body.slice(0, 300)}`)
    this.status = status
    this.body = body
  }
}

async function openLuxFetch(path: string, body: any, timeoutMs = 120000) {
  if (!API_KEY) throw new Error('OPENLUX_API_KEY not configured in .env.local')
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new OpenLuxHttpError(res.status, err)
    }
    const responseText = await res.text()
    if (!responseText || responseText.trim().length === 0) {
      throw new Error('OpenLux API returned empty response body')
    }
    try {
      return JSON.parse(responseText)
    } catch {
      throw new Error(`OpenLux API returned non-JSON: ${responseText.slice(0, 200)}`)
    }
  } catch (e) {
    clearTimeout(id)
    throw e
...
// 优先调用 MiniMax 官方 API（最高优先级），失败时回退到 OpenLux 聚合供应商。
...
export async function generateText(
  ...
  // step 3: OpenLux 聚合供应商降级
  const data = await openLuxFetch('/v1/chat/completions', {
    model,
...
export async function generateVisionText(params: GenerateVisionTextParams): Promise<string> {
  ...
  try {
    const data = await openLuxFetch('/v1/chat/completions', body)
    return data.choices[0].message.content
  } catch (err: any) {
    if (err?.message?.includes('does not support image input') || err?.status === 400) {
      ...
      const fallbackData = await openLuxFetch('/v1/chat/completions', {
        model: 'deepseek-chat',
 ...
 * 当模型 ID 以 'gemini' 开头时，不走 OpenLux 兼容的 /v1/images/generations，
 * 而是直接调用 Gemini 的 generateContent 端点。
 */
async function callGeminiImage(p: GenerateImageParams): Promise<OpenLuxImageRaw> {
  if (!API_KEY) {
    throw new Error('OPENLUX_API_KEY not configured')
  }
  ...
  if (!res.ok) {
    throw new OpenLuxHttpError(res.status, text)
  }
...
/** Resolve the provider-supported size for GPT Image models. */
function mapToGptImageSize(model: string, size?: string, aspectRatio?: string): string {
  const ar = aspectRatio || size || '1:1'
  // OpenLux documents these four values for gpt-image-1.
  if (model.toLowerCase() === 'gpt-image-1') {
    if (ar === 'auto' || ar === '1024x1024' || ar === '1536x1024' || ar === '1024x1536') return ar
    if (/^\d+x\d+$/i.test(ar)) {
    ...
    if (/^9:16|3:4|2:3|portrait/i.test(ar)) return '1024x1536'
    if (/^16:9|4:3|3:2|21:9|landscape/i.test(ar)) return '1536x1024'
    return '1024x1024'
  }
...
function resolveSize(model: string, aspectRatio?: string, explicitSize?: string): string {
  if (model.toLowerCase().includes('gpt-image')) return mapToGptImageSize(model, explicitSize, aspectRatio)
  // 已有像素尺寸直接返回
...
function buildPayload(p: GenerateImageParams): Record<string, any> {
  ...
  function normalizeRefs(): string[] {
    ...
    if (p.referenceImages && (p.referenceImages as any[]).length > 0 && refs.length === 0) {
      ...
      console.warn(`[OPENLUX-IMG] normalizeRefs: ${(p.referenceImages as any[]).length} 参考图均无效(非http/data), 第一个: ${sample}`)
    }
    return refs
  }
  ...
  if (!promptRaw || promptRaw.trim().length === 0) {
    console.error(`[OPENLUX-IMG] buildPayload 收到空 prompt，model=${p.model}`)
  }
  const prompt =
    promptRaw.length > PROMPT_MAX_LEN
      ? (() => {
          console.warn(
            `[OPENLUX-IMG] prompt 长度 ${promptRaw.length} 超过 ${PROMPT_MAX_LEN}，已软截断`
          )
  ...
  if (kind === 'doubao-multi') {
    ...
    if (allRefs.length > 0) {
      body.image = allRefs
      console.log(`[OPENLUX-IMG] 多图模型 ${p.model}：传入 ${allRefs.length} 张参考图`)
    } else {
      console.warn(`[OPENLUX-IMG] 多图模型 ${p.model}：无 http(s) 参考图，降级为纯文生图`)
    }
    return body
  }
  ...
  if (kind === 'doubao') {
    ...
    if (allRefs.length > 0) {
      ...
      console.log(`[OPENLUX-IMG] 单图模型 ${p.model}：传入 1 张参考图（共 ${allRefs.length} 张可用）`)
    }
    return body
  }
...
let _openLuxImageRequestCount = 0
...
async function callGptImageEdit(p: GenerateImageParams): Promise<OpenLuxImageRaw> {
  if (!API_KEY) throw new Error('OPENLUX_API_KEY not configured')
  ...
  if (imageCount === 0) {
    ...
    // Minimal valid white PNG; avoids an optional native canvas dependency in serverless builds.
    const blankPng = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==', 'base64')
  ...
  if (!res.ok) {
    ...
    throw new OpenLuxHttpError(res.status, errText)
  }
...
async function callOpenLuxImageOnce(p: GenerateImageParams): Promise<OpenLuxImageRaw> {
  ...
  if (!API_KEY || typeof API_KEY !== 'string' || API_KEY.trim().length === 0) {
    throw new Error('OPENLUX_API_KEY not configured — 真实 API 不可用')
  }

  _openLuxImageRequestCount++

  // 防御：空 prompt 直接拒绝，避免供应商侧用时 0s 的无效请求
  if (!params.prompt || params.prompt.trim().length === 0) {
    throw new Error(`OPENLUX_IMG_EMPTY_PROMPT: model=${params.model}，prompt 为空或仅含空白字符`)
  }
  ...
  // ========= [OPENLUX-IMG-REQUEST] =========
  console.log(`======== [OPENLUX-IMG-REQUEST #${_openLuxImageRequestCount}] ========`)
  ...
  const abortTimer = setTimeout(() => {
    console.error(`[OPENLUX-IMG-ABORT] fetch 超时（180s），强制中断。model=${params.model}`)
    controller.abort()
  }, 180000)
  ...
  // ========= [OPENLUX-IMG-RESPONSE] =========
  // 防御版：先用 text() 拿到原始响应，避免 JSON 解析失败时看不到内容
  const responseText = await res.text()
  console.log('======== [OPENLUX-IMG-RESPONSE] ========')
  console.log('Status:', res.status, res.statusText)
  ...
  if (!res.ok) {
    throw new OpenLuxHttpError(res.status, responseText)
  }
  ...
  try {
    ...
    throw new Error(
      `OpenLux API returned non-JSON: status=${res.status}, text=${responseText.slice(0, 200)}`
    )
  }

  if (!data || !Array.isArray(data.data) || data.data.length === 0) {
    throw new Error(
      `OPENLUX_IMAGE_PARSE_ERROR: 响应缺少 data 数组。model=${params.model}, response=${JSON.stringify(data).slice(0, 500)}`
    )
  }
  ...
  if (!url && !b64) {
    throw new Error(
      `OPENLUX_IMAGE_PARSE_ERROR: 无法从响应提取 url/b64_json。model=${params.model}, response=${JSON.stringify(data).slice(0, 500)}`
    )
  }
...
function isRetryableError(e: any): boolean {
  if (e instanceof OpenLuxHttpError) return RETRY_STATUS.includes(e.status)
...
async function _generateImageInner(params: GenerateImageParams): Promise<GenerateImageResult> {
  ...
  const hasGptImageRefs = !!(params.referenceImageUrl || params.referenceImages?.length)
  if (isGptImage && hasGptImageRefs) {
    ...
    try {
      ...
      if (params.requireReferenceImages && (params.referenceImageUrl || params.referenceImages?.length)) {
        ...
        try {
          const raw = await callOpenLuxImageOnce(stripped)
          const buffer = await rawToBuffer(raw)
          return { buffer, url: raw.url || '', model: params.model, revisedPrompt: raw.revisedPrompt, isMock: false }
      ...
      console.warn(`[GPT-EDIT] ${params.model} 失败，交给主循环尝试其他模型`)
    }
  }

  // Gemini 系列走原生 API，不经过 OpenLux 的 /v1/images/generations
    ...
    for (const tryModel of modelsToTry) {
      ...
      for (let attempt = 0; attempt <= BACKOFF_MS.length; attempt++) {
        try {
          ...
          const raw = await callOpenLuxImageOnce(subParams)
          const buffer = await rawToBuffer(raw)
          ...
          // 429 限流重试
          if (e instanceof OpenLuxHttpError && e.status === 429) {
            let retries = 0
            while (retries < RATE_LIMIT_MAX_RETRIES) {
              try {
                ...
                retries++
                if (retryErr instanceof OpenLuxHttpError && retryErr.status === 429 && retries < RATE_LIMIT_MAX_RETRIES) {
...
async function rawToBuffer(raw: OpenLuxImageRaw): Promise<Buffer> {
  if (raw.b64) {
  ...
  throw new Error('OPENLUX_IMAGE_PARSE_ERROR: raw has neither url nor b64')
}
...
export async function generateVideo(
  ...
  const data = await openLuxFetch('/v1/video/generations', {
    model,
    prompt,
    duration,
  })
 ...
 */
async function callOpenLuxVideoOnce(
  ...
  console.log('======== [OPENLUX-VIDEO-REQUEST] ========')
  ...
  const text = await res.text()
  console.log('======== [OPENLUX-VIDEO-RESPONSE] ========')
  console.log('Status:', res.status, res.statusText)
...
export async function generateVideoFromImage(
  ...
  // OpenLux Vidu Q2：默认图生视频协议。
  if (m.includes('vidu')) {
    const { videoUrl, taskId } = await generateViduVideo({
  ...
  const r = await callOpenLuxVideoOnce(params)
...
export async function submitJimengVideo(params: {
  ...
  if (!res.ok) throw new OpenLuxHttpError(res.status, text)
...
export interface ViduSubmitParams {
  prompt: string
  imageUrl: string
  model?: string
  duration?: number
...
/**
 * OpenLux Vidu 原生协议：POST /ent/v2/img2video。
 * 文档要求 images 仅包含一张公网 URL 或 Base64 图片，并异步返回 task_id。
 */
export async function submitViduVideo(params: ViduSubmitParams): Promise<SubmitTaskResult> {
  if (!API_KEY) throw new Error('OPENLUX_API_KEY not configured')
  ...
  const duration = Math.max(1, Math.min(10, Math.round(params.duration ?? 5)))
  const body = JSON.stringify({
    model: params.model || 'viduq2-turbo',
    images: [imageRef],
  ...
  const text = await res.text()
  console.log('[VIDU-SUBMIT] ←', res.status, text.slice(0, 500))
  if (!res.ok) throw new OpenLuxHttpError(res.status, text)
  ...
  try {
    ...
    throw new Error(`submitViduVideo: non-JSON response: ${text.slice(0, 200)}`)
  }

  const taskId: string | undefined = data?.task_id || data?.id || data?.data?.task_id
  if (!taskId) throw new Error(`submitViduVideo: 响应缺少 task_id: ${text.slice(0, 300)}`)
  return { taskId, status: data?.state || data?.status || 'created' }
}

/** OpenLux Vidu 任务查询：GET /ent/v2/tasks/{id}/creations。 */
export async function pollViduTask(
  taskId: string,
  pollTimeoutSec = 240,
  pollIntervalMs = 5000
): Promise<string> {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      const text = await res.text()
      if (!res.ok) {
        console.warn(`[VIDU-POLL] taskId=${taskId} HTTP ${res.status}: ${text.slice(0, 200)}`)
        continue
      }
      ...
      if (state !== lastState) {
        console.log(`[VIDU-POLL] taskId=${taskId} state=${state} hasUrl=${!!videoUrl}`)
        lastState = state
      }

      if (state === 'success' && videoUrl) return videoUrl
      if (state === 'failed' || state === 'error') {
        throw new Error(`Vidu task ${taskId} failed: ${text.slice(0, 300)}`)
      }
    } catch (err: any) {
      if (/Vidu task .* failed/i.test(err?.message || '')) throw err
      console.warn(`[VIDU-POLL] taskId=${taskId} 轮询异常:`, err?.message)
    }
  }

  throw new Error(`Vidu task ${taskId} timeout after ${pollTimeoutSec}s`)
}

export async function generateViduVideo(
  params: ViduSubmitParams & { pollTimeoutSec?: number; pollIntervalMs?: number }
): Promise<{ videoUrl: string; taskId: string }> {
  const { taskId } = await submitViduVideo(params)
  const videoUrl = await pollViduTask(
    taskId,
    params.pollTimeoutSec ?? 240,
    params.pollIntervalMs ?? 5000
  )
  return { videoUrl, taskId }
}
...
export async function generateMusicMinimax(params: {
  ...
  if (!apiKey) throw new Error(useOfficial ? 'MINIMAX_API_KEY not configured' : 'OPENLUX_API_KEY not configured')
...
export async function generateDirectVideo(params: GenerateDirectVideoParams): Promise<GenerateDirectVideoResult> {
  ...
  // 0) OpenLux Vidu Q2 Turbo：默认首帧图生视频。
  if (m.includes('vidu')) {
    if (hasLastFrame) {
      console.warn('[VIDEO-DIRECT] /ent/v2/img2video 仅使用首帧，忽略尾帧')
    }
    const { videoUrl, taskId } = await generateViduVideo({
      prompt,
      ...
      duration,
      resolution: '720p',
      pollTimeoutSec: Math.min(pollTimeoutSec, 240),
      pollIntervalMs,
    })
  ...
  // 2) Hailuo：支持 first_frame_image + last_frame_image
  ...
  // 未知模型：抛错
  throw new Error(`generateDirectVideo: 不支持的模型 ${model}。可用: viduq2-turbo / wan2.5-i2v-preview / minimax-hailuo-2.3 / veo3-* / jimeng-video`)
}

// ==================== GPT Image 图生图编辑（概念图用）====================
//
// 使用 /v1/images/edits 端点，gpt-image-1 模型，多图参考（styleRef + characterRef）
...
export async function generateConceptSceneWithEdit(params: {
  ...
  // gpt-image-1 的 edits 端点 image 字段只接受单张图，角色图以文字描述注入 prompt
