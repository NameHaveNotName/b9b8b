/**
 * 通过 openlux.ts 中的 generateMusicMinimax 测试 MiniMax 官方 API
 * 运行: npx tsx scripts/test-minimax-via-openlux.ts --yes
 */
...
;(async () => {
  ...
  // 动态导入,确保 dotenv 已先加载
  const { generateMusicMinimax } = await import('../lib/api-clients/openlux')
